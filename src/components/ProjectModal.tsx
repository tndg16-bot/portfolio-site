                ...
                <>
                    ...
                    >
                        <div
                            ...
                            className="w-full max-w-2xl bg-surface border border-japan-indigo/10 rounded-3xl p-6 shadow-2xl pointer-events-auto max-h-[85vh] overflow-y-auto scrollbar-custom"
                        >
                            ...
                            <div className="flex justify-between items-start mb-6">
                                    ...
                                    <div className="flex items-center gap-3 text-sm">
                                        <span className={`px-3 py-1 rounded-full border ${project.status === 'completed'
                                                ...
                                                : project.status === 'in_progress'
                                                    ...
                                                    : 'bg-surface-section border-border-default text-text-secondary'
                                            }`}>
                                ...
                                <button
                                    onClick={onClose}
                                    aria-label="閉じる"
                                    className="p-2 hover:bg-surface-section rounded-full transition-colors text-text-secondary hover:text-japan-indigo"
                                >
                                    <X size={24} />
                                </button>
                            </div>
                            ...
                            {project.subtasks && project.subtasks.length > 0 ? (
                                <div className="space-y-3">
                                    ...
                                    {project.subtasks.map((task, index) => (
                                        ...
                                        >
                                            ...
                                            <span className={`${task.completed ? 'text-text-secondary line-through' : 'text-text-secondary'}`}>
                                                {task.title}
                                            </span>
                                        </div>
                                    ))}
                                ...
                                <div className="text-center py-8 text-text-secondary bg-surface-alt rounded-xl border border-border-default">
                                    サブタスク情報はありません
                                </div>
                            )}
                            ...
                            {(project.issueUrl || project.issueNumber) && (
                                <div className="mt-8 pt-6 border-t border-japan-indigo/5 flex justify-end">
                                    <a
                                        ...
                                        className="text-sm text-text-secondary hover:text-japan-indigo transition-colors flex items-center gap-2"
                                    >
